...
import { bodyHeightForChrome, chromeForPageBlocks } from "#/lib/page-body-budget";
...
export default function FlowDocViewport({
  ...
  const pageFrames = useMemo((): PageFrame[] => {
    ...
    for (let i = 0; i < pages.length; i++) {
      ...
      const bodyHeightPx = bodyHeightForChrome(resolvedSetup, showHeader, showFooter);
  ...
  function focusBodyEditor() {
    const editor = document.querySelector<HTMLElement>("[data-doc-body-editor] .ProseMirror");
    editor?.focus();
  }

  function pageInsertGap(pageIndex: number) {
    if (!onPageInsert) return null;
    return <PageGapInsert onInsert={(block) => onPageInsert(pageIndex, block)} />;
  }

  if (!studioMode) {
    return (
      <div className="flow-doc-viewport flow-doc-viewport-read" style={{ width: contentWidthPx, margin: "0 auto" }}>
        {pageFrames.map((frame, i) => (
          <div key={`read-${frame.pageNum}`} className="studio-page-unit" style={{ marginTop: i > 0 ? pageGapPx : 0 }}>
            <div
            ...
            >
              {frame.showHeader && (
                <DocPageChrome
                  chrome={resolveDocChrome(chrome, { title, page: frame.pageNum, total })}
                  position="header"
                />
              )}
              <div className="doc-page-body">
                <DocRenderer blocks={frame.blocks} sections={sections} pageFragments={frame.fragments} />
              </div>
              {frame.showFooter && (
                <DocPageChrome
                  chrome={resolveDocChrome(chrome, { title, page: frame.pageNum, total })}
  ...
  return (
    <div className="flow-doc-viewport flow-doc-viewport-studio" style={{ width: contentWidthPx, margin: "0 auto" }}>
      {coverFrames.map((frame) => (
        <div key={`cover-${frame.pageNum}`} className="studio-page-unit" style={{ marginBottom: pageGapPx }}>
          <div
      ...
      <div className="studio-page-unit" style={{ marginTop: coverFrames.length > 0 ? pageGapPx : 0 }}>
        <div
        ...
        >
          {primaryBodyFrame.showHeader && (
            <ChromeComponent
              chrome={resolveDocChrome(chrome, { title, page: primaryBodyFrame.pageNum, total })}
              position="header"
              {...chromeProps}
            />
          )}
      ...
      {overflowFrames.map((frame) => (
        <div key={`overflow-${frame.pageNum}`} className="studio-page-unit flow-doc-overflow-page" style={{ marginTop: pageGapPx }}>
          <div
