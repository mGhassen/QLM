...
export default function LayoutSection({ blockType, props, onPropsChange }: LayoutSectionProps) {
  ...
  if (!showLayout && blockType !== "split" && blockType !== "levels") return null;
  ...
  return (
    <StyleSection title="Layout">
      {blockType === "grid" && (
        <WfFieldRow>
          <WfField label="Columns">
            <WfStepper value={cols} min={2} max={4} onChange={(v) => setProp("cols", v)} />
          </WfField>
          <WfField label="Rows">
            <WfStepper value={rows} min={1} max={4} onChange={(v) => setProp("rows", v)} />
          </WfField>
        </WfFieldRow>
      )}
          ...
          <WfField label="Align">
            <div className="wf-align-row">
                ...
                {ALIGN_GRID.map(({ align: a, justify: j }) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => onPropsChange({ ...props, align: a, justify: j })}
                    />
                  );
                })}
              </div>
              <div className="wf-align-selects">
                ...
                <WfSelect value={justify} onChange={(v) => setProp("justify", v)}>
                  <option value="start">Start</option>
                  <option value="center">Center</option>
                  <option value="end">End</option>
      ...
      {blockType === "levels" && (
        <>
          <WfField label={`Tab — ${((props.tabWidth as number) ?? 38).toFixed(0)}mm`}>
            <input
              type="range"
              min="28"
              max="60"
          ...
          {(((props.colFlex as number[] | undefined) ?? [1, 1]) as number[]).map((w, i, arr) => (
      ...
      {blockType === "split" && (
        <>
          <WfField label={`Main — ${((props.mainFlex as number) ?? 1.75).toFixed(2)}`}>
            <input
              type="range"
              min="0.5"
              max="3"
          ...
          <WfField label={`Rail — ${((props.railFlex as number) ?? 1).toFixed(2)}`}>
            <input
              type="range"
              min="0.5"
              max="3"
