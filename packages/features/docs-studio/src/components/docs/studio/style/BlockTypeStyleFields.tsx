  ...
  switch (block.type) {
      ...
      return (
        <WfField label="Preset">
          <WfSelect
            ...
            onChange={(v) => setProp('variant', v || undefined)}
          >
        ...
        <>
          ...
          <WfField label="Columns">
            <WfStepper
              ...
              max={4}
              onChange={(count) =>
                onBlockChange?.(setLevelsColumnCount(block, count))
              }
            />
          </WfField>
        </>
    ...
    case 'lcard':
      return (
        <WfField label="Tier">
          <WfSelect
            value={String(props.tier ?? 1)}
            onChange={(v) => setProp('tier', parseInt(v))}
          >
            {[1, 2, 3].map((n) => (
              <option key={n} value={n}>
                Tier {n}
