...
export type BlockNodeType = "paragraph" | "heading" | "bulletList" | "orderedList";
...
function toHexColor(value: string): string {
  ...
  const srgbMatch = trimmed.match(/^color\(\s*srgb\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)/);
  if (srgbMatch) {
...
export function setBlockAttr(editor: Editor, key: string, value: string) {
  ...
  editor.chain().focus().updateAttributes(type, { [key]: next }).run();
}
