...
function findPrintSource(): HTMLElement | null {
  return (
    document.querySelector<HTMLElement>(".studio-canvas .doc-shell.layout-paginated") ??
    document.querySelector<HTMLElement>(".doc-shell.layout-paginated") ??
    document.querySelector<HTMLElement>(".studio-canvas .doc-shell.layout-web > .doc-page") ??
...
function printCssFromShell(shell: HTMLElement | null) {
  ...
  const [top, right, bottom, left] = marginsRaw.split(",").map((v) => Number(v) || 0);
...
export async function exportDocAsPdf(title: string, sourceSelector?: string) {
  const source =
    (sourceSelector ? document.querySelector<HTMLElement>(sourceSelector) : null) ??
    findPrintSource();
