...
function flattenSectionChildren(
  ...
  for (const child of section.children ?? []) {
    if (child.type === 'break') {
      parts.push(
        `{% break variant="${(child.props?.variant as string) ?? 'page'}" /%}`,
      );
      continue;
    }
...
function flattenRootBlock(
  ...
  if (block.type === 'break') {
    return [
      `{% break variant="${(block.props?.variant as string) ?? 'page'}" /%}`,
    ];
  }
...
export function ensureFlowDocument(
  ...
  if (isFlowDoc(document)) {
    return document.body !== undefined ? document : { ...document, body: '' };
  }
