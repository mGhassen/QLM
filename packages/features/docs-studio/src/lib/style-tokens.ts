...
export function detectSpacingToken(
...
export function detectTextSizeToken(
  props: Record<string, unknown>,
): TextSizeToken {
...
export function spacingTokenToProps(
  token: SpacingToken,
...
export function widthTokenToProps(
  token: WidthToken,
): Record<string, string | undefined> {
  return { width: WIDTH_TOKEN_VALUES[token] };
}

export function textSizeTokenToProps(
  token: TextSizeToken,
...
export function blockSupportsWidth(type: BlockType): boolean {
  return ['hero', 'figure', 'card', 'alert', 'paragraph', 'quote'].includes(
    type,
  );
}
