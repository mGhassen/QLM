...
export const PAGE_FORMATS: Record<DocPageFormat, PageDimensions> = {
  ...
  letter: { widthMm: 216, heightMm: 279, label: "US Letter", printSize: "letter" },
  legal: { widthMm: 216, heightMm: 356, label: "US Legal", printSize: "legal" },
  tabloid: { widthMm: 279, heightMm: 432, label: "Tabloid", printSize: "tabloid" },
  executive: { widthMm: 184, heightMm: 267, label: "Executive", printSize: "executive" },
};

export function resolvePageFormat(format: DocPageFormat | undefined): PageDimensions {
  return PAGE_FORMATS[format ?? "a4"];
}

export function pageFormatToStyle(format: DocPageFormat | undefined): CSSProperties {
