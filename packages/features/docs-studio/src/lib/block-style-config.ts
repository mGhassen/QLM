...
const PRESET_BLOCKS: BlockType[] = ["card", "alert", "rail", "figure", "table", "level", "lcard", "levels"];

const LAYOUT_BLOCKS: BlockType[] = ["grid", "box", "section", "split", "levels"];

const CONTAINER_BLOCKS: BlockType[] = ["main", "flow", "vm", "levels", "level", "kpiband"];
...
export function getStylePanelSections(type: BlockType): StylePanelSection[] {
  ...
  if (type === "coverSubt" || PRESET_BLOCKS.includes(type)) sections.push("typePreset");
  if (LAYOUT_BLOCKS.includes(type) || type === "cover" || type === "coverBody") sections.push("layout");

  sections.push("spacing", "size");

  if (LAYOUT_BLOCKS.includes(type) || CONTAINER_BLOCKS.includes(type) || type === "hero") {
