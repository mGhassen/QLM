...
export function insertPageAfter(blocks: BlockNode[], afterPageId: string | null): {
  blocks: BlockNode[];
  focusId: string;
} {
  const newPage = createStarterPage();
  if (!afterPageId) {
    return { blocks: ensureCoverFirst([...blocks, newPage]), focusId: focusIdForPage(newPage) };
  }

  const newBlocks = insertBlockRelative(blocks, afterPageId, newPage, "after");
  return { blocks: ensureCoverFirst(newBlocks), focusId: focusIdForPage(newPage) };
}

export function insertSectionInPage(blocks: BlockNode[], pageId: string): {
  blocks: BlockNode[];
  focusId: string;
} {
  const section = createBlock("section");
  const newBlocks = insertBlockRelative(blocks, pageId, section, "inside");
  return { blocks: ensureCoverFirst(newBlocks), focusId: focusIdForSection(section) };
}
...
function insertNewSectionAtPageEnd(
  ...
  if (pageBlock) {
    ...
    const newBlocks = insertBlockRelative(blocks, pageBlock.id, section, "inside");
    return { blocks: ensureCoverFirst(newBlocks), focusId: focusIdForSection(section) };
  }

  if (!anchorId) {
    const page = createStarterPage();
    return { blocks: ensureCoverFirst([...blocks, page]), focusId: focusIdForPage(page) };
  }
...
export function findParentPageId(blocks: BlockNode[], childId: string, parentId: string | null = null): string | null {
...
export function insertAtPageEnd(
  ...
  if (newBlock.type === "section") {
    return insertNewSectionAtPageEnd(blocks, pageBlocks, pageInsertAnchor(pageBlocks));
  }
...
export function insertCoverPage(blocks: BlockNode[]): { blocks: BlockNode[]; focusId: string } {
  const withoutCover = blocks.filter((b) => !(b.type === "page" && isCoverPageBlock(b)) && b.type !== "cover");
...
export function findBlockPageIndex(pages: BlockNode[][], blockId: string): number {
...
export function canMoveBlockOnPages(
  ...
  if (pageIndex !== -1) {
    ...
    if (posOnPage !== -1) {
      ...
      if (direction === "down" && posOnPage === pageIds.length - 1 && pageIndex < pages.length - 1) {
        return true;
      }
    }
  }
...
export function scrollTreeItemIntoView(treeId: string) {
  requestAnimationFrame(() => {
    const el = document.querySelector<HTMLElement>(`[data-tree-id="${treeId}"]`);
    if (!el) return;
