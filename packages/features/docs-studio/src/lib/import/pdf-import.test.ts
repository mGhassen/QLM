...
describe("importPdf", () => {
  it("creates paragraph blocks from extracted text", async () => {
    ...
    const paragraphs = result.document.blocks.flatMap((page: BlockNode) =>
      (page.children ?? []).flatMap((section: BlockNode) => section.children ?? []),
    );

    expect(paragraphs.some((block: BlockNode) => block.type === "paragraph")).toBe(true);
    expect(paragraphs.some((block: BlockNode) => block.content?.includes("First paragraph line"))).toBe(
      true,
    );
  });
});
