...
export function getZoneRawContent(chrome: DocChrome | undefined, zone: ChromeZoneId): string {
...
export function resolveDocChrome(
  ...
  const legacyOff = (chrome as { enabled?: boolean } | undefined)?.enabled === false;
  ...
  return {
    showHeader: headerEnabled && !!(zoneHasContent(headerLeftHtml) || zoneHasContent(headerRightHtml)),
    showFooter: footerEnabled && !!(zoneHasContent(footerLeftHtml) || zoneHasContent(footerRightHtml)),
...
export function pageHasCover(blocks: { type: string; props?: Record<string, unknown> }[]): boolean {
