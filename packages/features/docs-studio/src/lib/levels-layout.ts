...
export function levelsColumnCount(block: BlockNode): number {
  ...
  return Math.max(
    ...rows.map(
      (r) => r.children?.filter((c) => c.type === 'lvlcol').length ?? 0,
    ),
    2,
  );
}
...
export function setLevelColumnCount(
  block: BlockNode,
  count: number,
): BlockNode {
...
export function setLevelsColumnCount(
  block: BlockNode,
  count: number,
): BlockNode {
  ...
  const clamped = Math.max(1, Math.min(4, count));
  const colFlex =
    (block.props?.colFlex as number[] | undefined) ?? Array(clamped).fill(1);
