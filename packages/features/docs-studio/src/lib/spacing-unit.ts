...
export const SPACING_UNITS: SpacingUnit[] = [
  'px',
  '%',
  'ch',
  'em',
...
export function resolveSpacingUnit(
...
export function spacingSliderConfig(
  unit: SpacingUnit,
  ...
  if (unit === 'mm') {
    return layer === 'margin'
      ? {
          ...
          max: 24,
          step: 0.5,
          presets: [-12, -6, 0, 6, 12, 18, 24] as number[],
        }
      : {
          min: -16,
          max: 16,
          step: 0.5,
          presets: [-8, -4, 0, 4, 8, 12, 16] as number[],
        };
  }
  if (unit === 'em' || unit === 'rem') {
    return {
  ...
  if (
    ...
    return {
      ...
      max: 100,
      step: 1,
      presets: [0, 5, 10, 20, 40, 60, 80, 100],
    };
  }
  if (unit === 'ch') {
