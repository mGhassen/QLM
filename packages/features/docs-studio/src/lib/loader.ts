...
function getAllDocsFromFilesystem(): DocListItem[] {
  ...
  return fs
    ...
    .filter((d) => d.isDirectory() && fs.existsSync(path.join(DOCS_DIR, d.name, "meta.json")))
    .map((d) => {
...
function loadDocFromFilesystem(slug: string): LoadedDoc | null {
  ...
  const rawDocument = JSON.parse(fs.readFileSync(docPath, "utf-8")) as DocDocument;

  const sections: Record<string, string> = {};
  const sectionsDir = path.join(dir, "sections");
  if (fs.existsSync(sectionsDir)) {
    for (const file of fs.readdirSync(sectionsDir).filter((f) => f.endsWith(".md"))) {
      sections[`sections/${file}`] = fs.readFileSync(path.join(sectionsDir, file), "utf-8");
    }
  }
