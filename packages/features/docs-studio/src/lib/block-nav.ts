...
export function focusBlockEditor(blockId: string, options?: { preventScroll?: boolean }) {
  const root = document.querySelector(`[data-block-id="${blockId}"]`);
  if (!root) return;
  const target =
    (root.querySelector(".ProseMirror[contenteditable='true']") as HTMLElement | null) ??
...
export function selectAllInBlockEditor(blockId: string): boolean {
  ...
  const textarea = root.querySelector("textarea.studio-popover-input") as HTMLTextAreaElement | null;
  if (textarea) {
    textarea.focus();
    textarea.select();
    return true;
  ...
  const editorWrap = root.querySelector("[data-field-editor]") as HTMLElement | null;
  if (editorWrap) {
    editorWrap.dispatchEvent(new CustomEvent("studio-select-all", { bubbles: false }));
    return true;
  }
