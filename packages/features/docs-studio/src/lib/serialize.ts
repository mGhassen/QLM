...
export function moveBlockToParent(
  ...
  if (
    ...
    !source ||
    source.type === 'cover' ||
    !canNestChild(target.type, source.type)
  ) {
    return document;
  }
...
export function insertBlockRelative(
  ...
  function insertInList(nodes: BlockNode[]): BlockNode[] {
    ...
    for (const node of nodes) {
      ...
      result.push(
        node.children
          ? { ...node, children: insertInList(node.children) }
          : node,
      );
  ...
  function moveInList(nodes: BlockNode[], atRoot: boolean): BlockNode[] {
    ...
    if (idx !== -1) {
      ...
      if (
        atRoot &&
        (block.type === 'cover' ||
          (block.type === 'page' && isCoverPageBlock(block)))
      ) {
        return nodes;
      }
      if (
...
export const BLOCK_DEFAULTS: Record<string, Partial<BlockNode>> = {
      ...
      { id: '', type: 'brand', content: 'QLM STRATEGIC RESEARCH' },
      {
        ...
        children: [
          ...
          },
          {
            ...
            content: 'Short description of the document.',
          },
          {
            id: '',
            type: 'grid',
  ...
  figure: {
    type: 'figure',
    content: 'src: /docs/placeholder.png\ncaption: Caption',
  },
  card: {
    type: 'card',
    props: { variant: 't-yellow' },
    content: '### New card\nCard content here.',
  },
  alert: {
    type: 'alert',
    props: { variant: 'insight', tag: 'Note' },
  ...
  kpiband: {
    ...
    children: [
      { id: '', type: 'kpi', props: { value: '440%', label: 'ROI sur 3 ans' } },
      {
        id: '',
        type: 'kpi',
        props: { value: '4 mois', label: 'seuil de rentabilité' },
      },
    ],
  },
  levels: {
