...
interface NotebookUIProps {
  ...
  /**
   * Cells for which an AI SQL-generation request is in flight. Drives the
   * spinner inside `NotebookCellAiPopup` and the cell-level "thinking" UI
   * so users get visible feedback during the LLM call.
   */
  aiLoadingCellIds?: Set<number>;
  /**
   * Per-cell partial assistant text streamed back from the AI generator.
   * Rendered live inside the popup so SQL appears token-by-token (same
   * pattern as the qwery agent's streaming assistant message).
   */
  aiStreamingTexts?: Map<number, string>;
  /**
   * Per-cell AI phase. Drives the prominent cell-level banner:
   *   'generating' — agent is drafting the SQL
   *   'executing'  — generated SQL is being run against the datasource
   * Cleared by the parent once both phases complete.
   */
  aiPhases?: Map<number, 'generating' | 'executing'>;
...
export function NotebookUI({
  ...
  const handleAddCell = (
    ...
    // Inherit the most recently-used datasource so the user doesn't
    // have to re-pick it on every new cell. Preference order:
    //   1. The cell we're inserting after (if any).
    //   2. The nearest prior cell (walking back from the bottom) that
    //      has a datasource set — handles the "add at bottom" case.
    //   3. The notebook's first registered datasource — fallback when
    //      no existing cell has picked one yet.
    //   4. Empty (current behavior).
    const pickDatasource = (): string[] => {
      const isValid = (ids: string[]) =>
        ids.length > 0 &&
        ids[0] !== undefined &&
        allDatasources.some((ds) => ds.id === ids[0]);

      if (afterCellId !== undefined) {
        const anchor = cells.find((c) => c.cellId === afterCellId);
        if (anchor && isValid(anchor.datasources))
          return [...anchor.datasources];
      }
      for (let i = cells.length - 1; i >= 0; i--) {
        const c = cells[i];
        if (c && isValid(c.datasources)) return [...c.datasources];
      }
      const first = allDatasources[0]?.id;
      return first ? [first] : [];
    };
  ...
  const handleRunQueryWithAgent = useCallback(
    (
      ...
      // Forward cellType so the host can branch: for prompt cells,
      // insert a fresh query cell below; for query cells, write into
      // the cell directly. Previously this was dropped silently and
      // every prompt cell ended up overwriting its own prompt with
      // generated SQL.
      onRunQueryWithAgent?.(cellId, query, datasourceId, cellType);
    },
    [onRunQueryWithAgent],
  );
            ...
            >
              <div className="flex flex-col gap-4">
                ...
                {cells.map((cell, index) => {
                  ...
                  // Get loading state for this cell. Either a SQL query is
                  // in flight (Run button) OR the AI is generating SQL for
                  // this cell — both should drive the spinner.
                  const isLoading =
                    (externalCellLoadingStates?.get(cell.cellId) ?? false) ||
                    (aiLoadingCellIds?.has(cell.cellId) ?? false);
                  ...
                  return (
                    <React.Fragment key={cell.cellId}>
                      <SortableCell
                        ...
                        onRunQuery={handleRunQuery}
                        onRunQueryWithAgent={handleRunQueryWithAgent}
                        aiStreamingText={aiStreamingTexts?.get(cell.cellId)}
                        aiPhase={aiPhases?.get(cell.cellId)}
