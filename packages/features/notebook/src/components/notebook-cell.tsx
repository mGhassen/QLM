...
import { extractStreamingSql } from './notebook-stream-utils';
...
interface NotebookCellProps {
  ...
  /** Live streaming text from the AI generator. Rendered inside the popup. */
  aiStreamingText?: string;
  /**
   * Drives the prominent cell-level banner that tells the user what the AI
   * pipeline is currently doing — generating the SQL or executing it. Set
   * to `undefined` once both phases complete.
   */
  aiPhase?: 'generating' | 'executing';
...
function NotebookCellComponent({
  ...
  // While the AI is generating SQL, mirror the streamed text into the
  // editor in real time so the user watches the query appear character
  // by character — same UX as a Claude Code / qwery code block.
  //
  // During the agent's probe phase the streamed text is empty (only
  // `[trace]` markers are flowing), and we still want the editor to
  // visibly "hand over" — otherwise the user sees the old query under
  // the "Generating SQL with AI…" banner and wonders if the AI did
  // anything. So while `aiPhase === 'generating'` the editor always
  // shows the streamed SQL, even when that's an empty string.
  //
  // Once `aiPhase` flips to `'executing'` or clears, we revert to
  // `localQuery` — by that point the mutation has landed and the
  // cell-sync `useEffect` has pulled the final SQL into local state.
  const streamingSql = useMemo(
    () =>
      aiPhase === 'generating' ? extractStreamingSql(aiStreamingText) : '',
    [aiPhase, aiStreamingText],
  );
  const query = aiPhase === 'generating' ? streamingSql : localQuery;
  ...
  const handlePromptSubmit = useCallback(() => {
    ...
    // Prompt cells now flow through the same SQL-generation pipeline
    // as query cells (the agent writes a fresh query cell underneath).
    // That pipeline requires a datasource — so DON'T bypass the
    // missing-datasource check for prompt cells anymore.
    ...
    <div className="group/cell relative flex w-full">
      ...
      >
        <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
          {aiPhase && (
            <div
              className="flex shrink-0 items-center gap-3 border-b border-yellow-500/60 bg-[#ffcb51] px-4 py-2.5 text-black"
              role="status"
              aria-live="polite"
              data-test="notebook-cell-ai-banner"
            >
              <Loader2 className="size-4 shrink-0 animate-spin" />
              <span className="text-xs font-bold tracking-wide">
                {aiPhase === 'generating'
                  ? t('cell.ai.generating', {
                      defaultValue: 'Generating SQL with AI…',
                    })
                  : t('cell.ai.executing', {
                      defaultValue: 'Running the generated query…',
                    })}
              </span>
              <span className="ml-auto text-[10px] font-medium tracking-wider text-black/70 uppercase">
                {aiPhase === 'generating' ? '1 / 2' : '2 / 2'}
              </span>
            </div>
          )}
            ...
            >
                ...
                {isEditingTitle ? (
                  ...
                  <>
                    ...
                    <span className="bg-muted text-muted-foreground shrink-0 rounded px-1.5 py-0.5 font-mono text-[10px] uppercase">
                      {t('cell.type.sql', { defaultValue: 'SQL' })}
                    </span>
                    <Button
                      size="icon"
                      variant="ghost"
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  {/* Wrapping span needed so the tooltip fires even when the
                      button is disabled (disabled buttons swallow pointer events). */}
                  <span className="shrink-0" tabIndex={0}>
                    <Button
                      size="sm"
                      onClick={handleRunQuery}
                      disabled={
                        !query.trim() || isLoading || !selectedDatasource
                      }
                ...
                <TooltipContent>
                  {!selectedDatasource
                    ? t('cell.actions.runPickDatasource', {
                        defaultValue: 'Select a datasource to run',
                      })
                    : t('cell.actions.run', {
                        defaultValue: 'Run cell (⌘↵)',
                      })}
                </TooltipContent>
              </Tooltip>
            </div>
            ...
            <div className="flex items-center gap-1">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      aria-label={t('cell.actions.toggleAi', {
                        defaultValue: 'Toggle AI assistant',
                      })}
                    >
                      <Sparkles className="size-3.5" />
                    </Button>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    onClick={onFormat}
                    aria-label={t('cell.actions.format', {
                      defaultValue: 'Format cell',
                    })}
                  >
                ...
                <TooltipContent>
                  {t('cell.actions.format', { defaultValue: 'Format cell' })}
                </TooltipContent>
              </Tooltip>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    aria-label={t('cell.actions.copy', {
                ...
                <TooltipContent>
                  {copySuccess
                    ? t('cell.actions.copied', { defaultValue: 'Copied' })
                    : t('cell.actions.copy', { defaultValue: 'Copy code' })}
                </TooltipContent>
              </Tooltip>
              <Tooltip>
                <TooltipTrigger asChild>
                  <span className="inline-flex" tabIndex={0}>
                    <Button
                      ...
                      aria-label={t('cell.actions.delete', {
                        defaultValue: 'Delete cell',
                      })}
                ...
                <TooltipContent>
                  {t('cell.actions.delete', { defaultValue: 'Delete cell' })}
                </TooltipContent>
              </Tooltip>
              ...
              <DropdownMenu>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <DropdownMenuTrigger asChild>
                      <Button
                        ...
                        aria-label={t('cell.actions.more', {
                  ...
                  <TooltipContent>
                    {t('cell.actions.more', { defaultValue: 'More actions' })}
                  </TooltipContent>
                </Tooltip>
                <DropdownMenuContent align="start">
                  {isQueryCell &&
                    (!cell.title || cell.title.trim().length === 0) && (
                      ...
                      >
                        ...
                        {t('cell.actions.addTitle', {
                          defaultValue: 'Add cell title',
                        })}
                      </DropdownMenuItem>
                    )}
                  <DropdownMenuItem
                  ...
                  >
                    ...
                    {t('cell.actions.duplicate', {
                      defaultValue: 'Duplicate cell',
                    })}
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onClick={onMoveUp}
                  ...
                  >
                    ...
                    {t('cell.actions.moveUp', { defaultValue: 'Move up' })}
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onClick={onMoveDown}
                    disabled={totalCellCount === 1}
                  ...
                  >
                    ...
                    {t('cell.actions.moveDown', { defaultValue: 'Move down' })}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={onFullView}>
                    <Maximize2 className="mr-2 h-4 w-4" />
                    {t('cell.actions.fullView', { defaultValue: 'Full view' })}
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
          ...
          {/* Results - Table/Graphs/API/Data App/Report.
              Height fits content (rows × ~40px + chrome) capped at
              480px so a single-row result doesn't waste a screen of
              vertical space. Non-table views fall back to a fixed
              height since their content size is unknown. */}
          {isQueryCell &&
            result &&
            (() => {
              const ROW_PX = 40;
              const CHROME_PX = 96; // grid header strip + table header + borders
              const MIN_PANEL_PX = 140;
              const MAX_PANEL_PX = 480;
              const visibleRows = Math.min(result.rows?.length ?? 0, 10);
              const fittedTableHeight = Math.min(
                MAX_PANEL_PX,
                Math.max(MIN_PANEL_PX, CHROME_PX + visibleRows * ROW_PX),
              );
              const panelHeight =
                resultView === 'table' ? fittedTableHeight : 400;
              return (
                <div
