...
interface NotebookUIProps {
   ...
   * spinner inside `NotebookCellAiPopup` so the user gets visible feedback
   * during the LLM call. Cleared by the parent when the call resolves.
   */
   ...
   *   'executing'  — "Running the generated query…"
   * Cleared by the parent when both phases complete.
   */
...
export function NotebookUI({
  ...
  // Re-sync `cells` from the notebook prop when the server-side data
  // changes (e.g. after an AI-generated SQL writes back into the cell).
  // Without this, mutations triggered outside the editor (plugin-root
  // updateNotebookMutation) refetch the notebook but the local cells
  // state stays at the value from initial mount.
  //
  // Tracked by `notebook.updatedAt` — any save bumps it, including the
  // user's own typing-driven autosave (which has already pushed the same
  // cell values into local state, so the re-sync is a no-op there).
  const lastSyncedAt = React.useRef<string | null>(null);
  React.useEffect(() => {
    if (!notebook?.cells) return;
    const stamp =
      notebook.updatedAt instanceof Date
        ? notebook.updatedAt.toISOString()
        : String(notebook.updatedAt ?? '');
    if (stamp === lastSyncedAt.current) return;
  ...
  }, [notebook?.cells, notebook?.updatedAt]);
  ...
  const handleRunQueryWithAgent = async (
    ...
    // Keep the popup open during the LLM call so the spinner (driven by
    // `aiLoadingCellIds` from the parent) is visible. Close ONLY on
    // success — keeping it open on failure lets the user see the error
    // toast and retry without re-typing.
    try {
      await onRunQueryWithAgent?.(cellId, query, datasourceId);
      handleCloseAiPopup();
    } catch {
      // Errors are toast'd by the parent. Popup stays open for retry.
    }
  };
            ...
            <div className="flex flex-col">
              <AnimatePresence mode="popLayout">
                {cells.map((cell, index) => {
                  ...
                  // Either: a SQL query is in flight (run-button) OR the
                  // AI popup is generating SQL for this cell.
                  const isLoading =
