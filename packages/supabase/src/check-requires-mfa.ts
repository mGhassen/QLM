...
// `suppressGetSessionWarning` is a private auth-js field used to silence the
// getSession warning during the MFA assurance check.
// https://github.com/supabase/auth-js/issues/873
type AuthWithSuppressWarning = SupabaseClient['auth'] & {
...
function setSuppressGetSessionWarning(
  ...
  (client.auth as AuthWithSuppressWarning).suppressGetSessionWarning = value;
}
...
export async function checkRequiresMultiFactorAuthentication(
  ...
  setSuppressGetSessionWarning(client, true);

  const assuranceLevel = await client.auth.mfa.getAuthenticatorAssuranceLevel();

  setSuppressGetSessionWarning(client, false);
