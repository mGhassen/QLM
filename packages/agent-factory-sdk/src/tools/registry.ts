...
// Each tool is `ToolInfo<SpecificSchema>`, but `tools` stores them as
// `ToolInfo<z.ZodType>`. Zod's `Params` is invariant in the execute signature,
// so we register through a generic helper that bridges the variance once.
function registerTool<Params extends z.ZodType>(t: ToolInfo<Params>): void {
  tools.set(t.id, t as ToolInfo);
}
...
function registerTools() {
  ...
  //registerTool(TestConnectionTool);
  registerTool(GetSchemaTool);
  registerTool(RunQueryTool);
  registerTool(RunQueriesTool);
  registerTool(SelectChartTypeTool);
