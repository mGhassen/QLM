...
import { CheckIcon, XIcon, ArrowDownIcon, PencilIcon } from 'lucide-react';
...
function QweryAgentUIContent(props: QweryAgentUIProps) {
  ...
  const [state, setState] = useState(() => {
    ...
    if (typeof window !== 'undefined') {
      try {
        webSearch =
          localStorage.getItem('guepard-web-search-enabled') === 'true';
      } catch {
        /* ignore */
      }
    }
  ...
  const [showSuggestionBadges, setShowSuggestionBadges] = useState(() => {
    ...
    try {
      const v = localStorage.getItem('guepard-show-suggestion-badges');
      return v !== 'false';
    } catch {
      return true;
    }
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(
        'guepard-show-suggestion-badges',
  ...
  const [preferredSearchEngine, setPreferredSearchEngine] =
    useState<SearchEngine>(() => {
      ...
      try {
        const v = localStorage.getItem('guepard-preferred-search-engine');
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(
        'guepard-preferred-search-engine',
      ...
      if (initialMessages && initialMessages.length > 0) {
        ...
        if (!idsMatch) {
          ...
          if (initialHasToolOutputs && !currentHasToolOutputs) {
            ...
            // Same message IDs, no tool-output delta — but metadata (e.g.
            // feedback thumbs state) may have changed server-side after a
            // POST /api/feedback + invalidateQueries. Full setMessages would
            // clobber in-flight streamed content; patch metadata in place
            // instead so the thumbs refresh without touching parts.
            const needsMetadataSync = initialMessages.some((initial) => {
              const current = messages.find((m) => m.id === initial.id);
              if (!current) return false;
              return (
                JSON.stringify(initial.metadata) !==
            ...
            if (needsMetadataSync) {
              setMessages((prev) =>
                prev.map((m) => {
                  ...
                  if (!initial) return m;
                  return { ...m, metadata: initial.metadata };
                }),
              );
            }
          }
                                            ...
                                            <div className="group w-full max-w-full min-w-0">
                                              ...
                                              >
                                                <Textarea
                                                  ...
                                                  className="bg-muted/50 text-foreground border-primary/30 focus:border-primary min-h-[60px] w-full resize-none rounded-lg border px-4 py-3 text-sm focus:outline-none"
                                                  onKeyDown={(e) => {
                                                    if (
                                                      e.key === 'Enter' &&
                                                      (e.metaKey || e.ctrlKey)
