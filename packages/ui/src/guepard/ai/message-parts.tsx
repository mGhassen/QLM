    ...
    <div className="flex flex-col gap-1">
      ...
      >
        <TaskItemIndicator
          status={task.status}
          className={cn(
            'self-stretch transition-colors duration-200',
            isSubstep ? 'min-h-3 w-0.5' : 'min-h-4 w-1',
          )}
        />
    ...
    >
      <HeadingContext.Provider value={headingContextValue}>
        ...
        >
          ...
          {/* Agent response actions are handled by `MessageItem` (copy, regenerate, menu). */}
        </Message>
  ...
  function TodoRow({
    ...
    return (
      ...
      >
        <div
          className={cn(
            'flex shrink-0 items-center justify-center rounded-md p-1.5 shadow-sm transition-colors duration-200',
            meta.badgeClass,
    ...
    if (part.type === 'tool-runQueries') {
      ...
      const headerInfo = (
        <div className="flex items-center gap-2">
          {isInProgress && (
            <div className="bg-muted/50 text-muted-foreground animate-in fade-in zoom-in flex items-center gap-1.5 rounded-full px-2 py-0.5 duration-300">
                    ...
                    return (
                      ...
                      >
                        <CollapsibleTrigger className="flex w-full cursor-pointer items-center gap-2 px-2 py-0.5 text-left">
                          <div className="scrollbar-hover-visible min-w-0 flex-1 overflow-x-auto">
                            {hasBatchOutput ? (
                              <CodeBlock
                                ...
                                disableHover={true}
                                noInternalScroll
                                preClassName="[&>pre]:!bg-transparent"
                                className="w-max min-w-0 !overflow-visible border-none !bg-transparent p-0"
                              />
                            ) : (
                          ...
                          {isCurrent && (
                            <CircleDashedIcon className="text-muted-foreground h-2.5 w-2.5 shrink-0 animate-spin" />
                          )}
                          <ChevronDownIcon
                            className={cn(
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        runQueriesAllOpen === false &&
                          'bg-muted/50 border-border text-muted-foreground shadow-sm',
                      )}
                      title={
                  ...
                  return (
                    <Collapsible
                      ...
                      className={cn(
                        ...
                        isExecuting &&
                          'border-border ring-border/40 bg-muted/20 shadow-lg ring-2',
                        success === false &&
                          ...
                          <div className="flex-shrink-0">
                            {isExecuting ? (
                              <div className="bg-muted/50 rounded-full p-1">
                                <CircleDashedIcon className="text-muted-foreground h-4 w-4 animate-spin" />
                              </div>
                            ) : success === true ? (
                                ...
                                {queryText && (
                                  ...
                                  >
                                    <CodeBlock
                                      ...
                                      disableHover={true}
                                      preClassName="[&>pre]:!bg-transparent"
                                      className="border-none !bg-transparent p-0 text-[10px] [&_pre]:!truncate [&_pre]:!py-1.5 [&_pre]:!text-[10px]"
                                    />
                                  </div>
                                )}
                                {meta.isDuplicate && (
