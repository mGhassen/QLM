...
import { shouldInvertDatasourceIcon } from '@guepard/shared/utils';
        ...
        <div className="group border-border bg-background/50 ring-offset-background hover:bg-background relative flex h-7 cursor-pointer items-center gap-2 rounded-md border pr-3 pl-1.5 text-xs backdrop-blur-sm transition-all hover:shadow-sm">
          <div className="flex -space-x-2">
            {stackIcons.map((icon, i) => (
              <div
                key={i}
                className="border-background/50 bg-muted flex h-5 w-5 items-center justify-center rounded border"
            ...
            {datasources.length > 3 && (
              <div className="border-background/50 bg-muted z-0 flex h-5 w-5 items-center justify-center rounded border text-[8px] font-bold">
                +{datasources.length - 3}
              </div>
            )}
          </div>
