...
import { DoseDataTable, type DoseDataTableRow } from './data-table-dose';

const meta: Meta<typeof DoseDataTable> = {
  title: 'Design System/DataTable (Dose port)',
  component: DoseDataTable,
  parameters: {
    layout: 'padded',
    docs: {
      description: {
        component:
          'Dose-style DataTable, ported as-is for evaluation. Storybook-only — not exported from package.json. Prefer `@guepard/ui/data-table-advanced` for production.',
      },
    },
  },
};
...
export default meta;
type Story = StoryObj<typeof DoseDataTable>;

type Node = DoseDataTableRow & {
  name: string;
  region: string;
  cpu: number;
...
export const Basic: Story = {
  ...
  render: () => (
    <div className="h-[500px]">
      <DoseDataTable<Node> data={NODES} columns={columns} title="Basic" />
    </div>
  ),
};
