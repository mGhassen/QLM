...
import { toToolError, toUserFacingError } from '../qlm/ai/user-facing-error';
...
export const ToolHeader = ({
  ...
  if (isMinimal) {
    return (
      ...
      >
        ...
        <div className="text-muted-foreground flex size-4 shrink-0 items-center justify-center transition-opacity duration-200 group-hover/header:opacity-80">
          {toolIcon}
        </div>
  ...
  return (
    <CollapsibleTrigger
      ...
      {...props}
    >
      <div className="from-muted/80 via-muted/50 to-muted/40 text-muted-foreground ring-border/60 flex size-11 shrink-0 items-center justify-center rounded-none bg-linear-to-br shadow-sm ring-1">
        {toolIcon}
      </div>
      ...
      <div className="flex min-w-0 flex-1 items-center gap-3">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div
            className={cn(
              'flex items-center gap-2 rounded-none px-3 py-1.5 text-xs font-medium transition-all',
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          {executionTimeLabel ? (
            <Badge
              variant="secondary"
              className="text-muted-foreground bg-muted/70 border-border/60 rounded-none px-2 py-1 text-xs font-medium tabular-nums"
            >
              {executionTimeLabel}
            </Badge>
          ) : null}

          <div className="bg-muted/50 group-hover/header:bg-muted flex size-8 shrink-0 items-center justify-center rounded-none transition-colors">
  ...
  if (isTestConnection && !errorText) {
    ...
    return (
      <div className={cn('border-t-2 px-5 py-5', className)} {...props}>
        <div
          className={cn(
            'flex items-center gap-4 rounded-none px-5 py-4 text-sm font-medium shadow-sm',
            result
              ? 'bg-linear-to-r from-emerald-50 to-emerald-100 text-emerald-700 ring-2 ring-emerald-200 dark:from-emerald-950 dark:to-emerald-900 dark:text-emerald-300 dark:ring-emerald-800'
              : 'bg-linear-to-r from-red-50 to-red-100 text-red-700 ring-2 ring-red-200 dark:from-red-950 dark:to-red-900 dark:text-red-300 dark:ring-red-800',
          )}
        >
          {result ? (
  ...
  if (errorText) {
    ...
    return (
      <div
        ...
        {...props}
      >
        <div className="rounded-none bg-linear-to-br from-red-50 via-red-50 to-orange-50 p-5 ring-2 ring-red-200 dark:from-red-950 dark:via-red-950 dark:to-orange-950 dark:ring-red-800">
          <div className="text-destructive mb-3 flex items-center gap-2 text-sm font-bold">
            <AlertCircleIcon className="size-5" />
            <span>Execution Error</span>
          </div>
          <div className="bg-background/80 rounded-none p-4 backdrop-blur-sm">
