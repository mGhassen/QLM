...
export async function highlightCode(
  ...
  return [stripPreBackground(light), stripPreBackground(dark)];
}
...
export const CodeBlock = ({
  ...
  return (
    <CodeBlockContext.Provider value={{ code }}>
      <div
        className={cn(
          'group text-foreground relative w-full min-w-0 overflow-hidden rounded-none',
          isSQL ? 'dark:bg-muted/30 bg-transparent' : 'bg-muted/30',
          className,
        )}
        {...props}
      >
        ...
        >
          <div
            className={cn(
              ...
              'dark:hidden',
              !preClassName &&
                (isSQL ? '[&>pre]:bg-transparent!' : '[&>pre]:bg-muted/30!'),
              preClassName,
            )}
