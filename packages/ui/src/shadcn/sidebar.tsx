...
>(
  (
    ...
    const storeOpen = useSidebarStore(
      (s) => s.isOpen || (s.settings.isHoverOpen && s.isHover),
    );
    const setIsOpenStore = useSidebarStore((s) => s.setIsOpen);
    const toggleOpenStore = useSidebarStore((s) => s.toggleOpen);
    const sidebarWidthPx = useSidebarStore((s) => s.width);

    React.useEffect(() => {
      setIsHydrated(true);
    }, []);

    const open = isHydrated ? storeOpen : (defaultOpen ?? true);
    const state = open ? 'expanded' : 'collapsed';

    const setOpen = useCallback<SidebarContextProps['setOpen']>(
      (value: boolean) => {
        if (isHydrated) {
          setIsOpenStore(value);
        }
        setOpenProp?.(value);
      },
      [isHydrated, setIsOpenStore, setOpenProp],
    );
    ...
    }, [isMobile, isHydrated, toggleOpenStore]);
    ...
    const resolvedSidebarWidthPx = isHydrated
      ? sidebarWidthPx
      : SIDEBAR_DEFAULT_WIDTH_PX;
      ...
      <SidebarContext.Provider value={contextValue}>
        <TooltipProvider delayDuration={0}>
          <div
            ...
            style={
              {
                '--sidebar-width': `${resolvedSidebarWidthPx}px`,
