...
export function TaskDelimiter({
  ...
  const progressLabel = totalCount > 0 ? `${doneCount}/${totalCount}` : null;
      ...
      >
        <HoverCardTrigger asChild>
          <button
            ...
            onClick={handleClick}
            className={cn(
              'text-muted-foreground hover:text-foreground flex max-w-full min-w-0 cursor-pointer items-center gap-2 overflow-hidden py-2 text-xs transition-colors',
              className,
            )}
        ...
        >
          ...
          {todos.length > 0 ? (
            <div className="text-muted-foreground/70 mt-2 flex items-center justify-between gap-2 border-t pt-2 text-xs">
