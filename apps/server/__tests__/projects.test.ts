...
describe('Server API – Projects', () => {
  ...
  describe('Projects CRUD', () => {
    it('GET /api/projects requires orgId', async () => {
      ...
      // zValidator returns Zod's issues shape; check for the field
      // anywhere in the response body rather than a specific message.
      expect(JSON.stringify(await res.json())).toMatch(/orgId|Organization/i);
    });
  ...
  describe('Projects bulk', () => {
    it('POST /api/projects/bulk with invalid body returns 400', async () => {
      ...
      expect(res.status).toBe(400);
      // zValidator returns Zod's issues shape; status alone is the
      // contract — body shape is library-specific.
    });
