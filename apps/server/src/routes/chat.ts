...
/**
 * Checks if the organization has remaining credits balance.
 * Returns an HTTP 402 response if balance is insufficient, otherwise null.
 */
async function assertOrganizationHasBalance(
  c: Context,
  organizationId: string,
  organizationRepository: IOrganizationRepository,
): Promise<Response | null> {
  const billingData =
    await organizationRepository.getBillingData(organizationId);
  if (billingData.balance <= 0) {
    return c.json(
      {
        error:
          'Insufficient credits. Please add credits to your organization to continue.',
      },
      402,
    );
  }
...
export function createChatRoutes(
  ...
  app.post(
    ...
    async (c) => {
      ...
      const balanceError = await assertOrganizationHasBalance(
        c,
