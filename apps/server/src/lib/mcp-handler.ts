...
const mcpRequestStorage = new AsyncLocalStorage<Request>();
...
async function createNewSession(
  getRepositoriesOverride?: (req: Request) => Promise<Repositories>,
  ...
  const getRepos = () => {
    if (getRepositoriesOverride) {
      const request = mcpRequestStorage.getStore();
      if (!request) {
        throw new Error('MCP request context not available');
      }
      return getRepositoriesOverride(request);
    }
    const request = mcpRequestStorage.getStore();
    if (!request) {
      throw new Error('MCP request context not available');
    }
    return getRepositoriesFromRequest(request);
  };
  const mcpServer = createMcpServer(getRepos);
...
export type HandleMcpRequestOptions = {
  getRepositories?: (req: Request) => Promise<Repositories>;
};
  ...
  if (existing) {
    const parsedBody =
      request.method === 'POST'
        ...
        : undefined;
    return mcpRequestStorage.run(request, () =>
      existing.transport.handleRequest(request, { parsedBody }),
    );
  }

  const isInit = await isInitializeRequest(request);
  if (isInit) {
    const entry = await createNewSession(options?.getRepositories);
    const parsedBody = await request.json().catch(() => undefined);
    const response = await mcpRequestStorage.run(request, () =>
