 ...
 * We create the schema for the subscriptions. Subscriptions are the subscriptions for an organization to a product. For example, an organization might have a subscription to a product with the status 'active'.
 * -------------------------------------------------------
...
comment on table public.subscriptions is 'The subscriptions for an organization';

comment on column public.subscriptions.organization_id is 'The organization the subscription is for';

comment on column public.subscriptions.customer_id is 'The billing-provider customer reference (e.g. Stripe cus_…) the subscription belongs to';
...
create index ix_subscriptions_organization_id on public.subscriptions (organization_id);
...
-- Users can read organization subscriptions on an organization they are a member of
...
-- Check if an organization has an active subscription - ie. it's trialing or active
-- Useful to gate access to features that require a subscription
create
or replace function public.has_active_subscription (target_organization_id uuid) returns boolean
set
  search_path = '' as $$
begin
    return exists (
