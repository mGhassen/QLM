/*
 * -------------------------------------------------------
 * Section: Prediction snapshots & agent conversations (RFC 0030 — phase 1)
 *
 * Three append-only tables that back the Predictions app:
 *   - public.prediction_schema_snapshots: immutable per-datasource snapshots
 *     of `IDataSourceDriver.metadata()`. Versioned monotonically per
 *     datasource. RLS is project-org scoped via `has_role_on_organization`.
 *   - public.prediction_agent_conversations: a chat session pinned to one
 *     snapshot. Owner-writable, org-readable.
 *   - public.prediction_agent_messages: append-only message history.
 *
 * No new app_permissions enum value: snapshot create + agent use both gate
 * on `datasources.read` (already granted to org members). RFC 0030 §8.
 * -------------------------------------------------------
 */
...
comment on table public.prediction_schema_snapshots is
  'Immutable, versioned snapshots of a datasource''s schema metadata. Append-only.';
...
comment on table public.prediction_agent_conversations is
  'A chat session pinned to a single prediction schema snapshot.';
...
comment on table public.prediction_agent_messages is
  'Append-only message history for prediction agent conversations.';
