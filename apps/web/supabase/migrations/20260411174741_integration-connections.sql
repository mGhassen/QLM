-- Integration connections table + RLS + triggers.
--
-- See docs/rfcs/0001-integrations.md and
-- apps/web/supabase/schemas/35-integration-connections.sql for context.
--
-- The `integrations.manage` permission referenced in the policies below
-- was added by migration 20260411174740_add-integrations-manage-permission.sql.
...
-- Revoke default permissions, then grant what authenticated needs.
      ...
      where p.id = integration_connections.project_id
        and (
          ...
          public.has_permission(
            auth.uid(),
            p.organization_id,
            'integrations.manage'::public.app_permissions
          )
        )
    )
  );
...
-- Seed the `integrations.manage` permission onto the existing owner and
-- administrator roles. Idempotent via `on conflict do nothing` so this
-- migration can run cleanly on a freshly-reset database (where the
-- 20-roles-seed schema file already includes the new value) or on an
-- existing database where the seed migration is older and pre-dates the
-- new enum value.
insert into public.role_permissions (role, permission) values
  ('owner', 'integrations.manage'),
  ('administrator', 'integrations.manage')
