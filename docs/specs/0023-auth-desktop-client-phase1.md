...
- [x] Same-origin webview ↔ sidecar at `127.0.0.1:<port>` (cookies scoped to that origin, never leave host). Shipped story 002; story 012 bridged apps/web via `window.__GUEPARD_API_URL` preserving this.
- [x] Sidecar binds `127.0.0.1` only — assert in `apps/server` startup when `GUEPARD_RUNTIME=desktop`. Shipped story 007 (`resolveDesktopRuntime` in `apps/server/src/runtime/desktop-runtime.ts`).
- [x] Keyring-IPC authenticated by per-launch one-time bearer token (`GUEPARD_KEYRING_TOKEN`); token never logged. Shipped stories 004 + 007 (`ipc.rs` bearer check + `lib.rs` `redact_for_log` OnceLock).
