# Spec — Desktop client auth (phase 1)
...
| - | ------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 | Sidecar → Tauri keychain transport — localhost IPC with one-time shared token, or renderer-mediated bridge?                                       | **Localhost IPC, one-time token.** Tauri binds an HTTP listener on a random localhost port at startup. Sidecar receives the port via `QLM_KEYRING_PORT` and a per-launch one-time token via `QLM_KEYRING_TOKEN`. Every keychain call sends `Authorization: Bearer <QLM_KEYRING_TOKEN>`; Tauri rejects requests with a missing/wrong token. Decouples sidecar boot-time refresh from webview readiness. |
...
| 7 | Renderer architecture — full Bun sidecar or static-renderer with bearer tokens to remote API?                                                       | **Full Bun sidecar.** Lifted verbatim from qwery-core: `pick_port(4096)` fallback, PID file at `app_config_dir/api-server.pid`, kill-previous on launch, Windows env-subset, server-ready TCP loop, log forwarding. Renderer loads `http://127.0.0.1:<port>` from a `window.__QLM_API_URL` injection. Static-renderer rejected because the agent loop must execute SQL tools locally.            |
| 8 | LLM call routing in phase 1?                                                                                                                       | **Direct providers only (BYO keys).** `packages/agent-factory-sdk` keeps its existing `@ai-sdk/anthropic`, `@ai-sdk/openai`, `@ai-sdk/azure`, `@ai-sdk/amazon-bedrock`, `@ai-sdk/openai-compatible` (Ollama) providers — all Vercel AI SDK v6 / `LanguageModelV2`. Provider keys live in the OS keychain via Rust `keyring` commands and are injected as env vars into the Bun sidecar at spawn. Cloud `qlmCloud()` provider is a later phase. |
| 9 | First-run picker UX?                                                                                                                                | **Full-screen blocking page.** Tauri webview loads `/desktop/first-run` if no `QLM_SERVER_URL` is set; user must pick before anything else loads. MDM-supplied URL pre-selected if present.                                                                                                                                                                                                       |
...
- As a desktop user, I can sign in against `api.qlm.com` (the default cloud server) using my existing Supabase credentials, via the in-app sign-in screen served by the local sidecar.
...
- As a developer testing on-prem with a self-signed cert, I can opt in via `allowInsecureTls: true` in `config.json` (or `QLM_ALLOW_INSECURE_TLS=1`) and see a clear banner that this is a non-production setting.
...
- `/desktop/first-run` — full-screen blocking page; only routable when no `QLM_SERVER_URL` is configured.
...
2. `QLM_SERVER_URL` not set → Tauri spawns sidecar with `QLM_FIRST_RUN=1`; webview loads `/desktop/first-run`.
3. User selects Cloud and clicks Continue → renderer calls `set_app_config({ QLM_SERVER_URL: "https://api.qlm.com" })` → Tauri writes to `config.json` and triggers `restart_sidecar`.
4. Sidecar restarts with `QLM_SERVER_URL` set → webview reloads → redirected to `/auth/sign-in`.
...
1. App opens → Tauri reads `config.json`, binds keyring IPC, reads `refresh_token:<QLM_SERVER_URL>` from the keychain.
2. Tauri spawns sidecar with `QLM_REFRESH_TOKEN=<value>` env var (in addition to URL + LLM keys + IPC env).
...
3. Renderer calls `set_app_config({ QLM_SERVER_URL: <new-url> })` → Tauri writes config, calls `restart_sidecar`.
...
| Scenario | Behaviour |
| -------- | --------- |
| OS keychain unavailable (no daemon, locked) | Tauri `get_api_key` returns error; sidecar boot proceeds without `QLM_REFRESH_TOKEN`; webview lands on sign-in screen. Banner: "Could not access OS keychain — sign-in required this session." |
...
spawn sidecar(QLM_RUNTIME=desktop,
              QLM_SERVER_URL=<url>,
              QLM_REFRESH_TOKEN=<r>,
              QLM_KEYRING_PORT=<p>,
              QLM_KEYRING_TOKEN=<t>,
...
- **Sidecar** (`apps/server` running with `QLM_RUNTIME=desktop`) — owns: auth flow, Supabase calls (sign-in, refresh, sign-out), agent loop, SQL tool execution, local SQLite session cache. Calls Tauri shell IPC only for keychain ops.
...
type ServerConfig = {
  QLM_SERVER_URL?: string;
  QLM_TELEMETRY_ENABLED?: 'true' | 'false';
...
| Tauri keyring IPC | POST   | `/keyring/set`                 | `Bearer <QLM_KEYRING_TOKEN>` | Write `key:value` to OS keychain.                             |
| Tauri keyring IPC | GET    | `/keyring/get?key=...`         | `Bearer <QLM_KEYRING_TOKEN>` | Read by key.                                                  |
| Tauri keyring IPC | DELETE | `/keyring/{key}`               | `Bearer <QLM_KEYRING_TOKEN>` | Delete by key.                                                |
...
- **Sidecar startup retry budget**: if Supabase is unreachable on boot, sidecar retries refresh 3 times with exponential backoff, then proceeds without `QLM_REFRESH_TOKEN` (webview lands on sign-in).
...
`app_config_dir/config.json` (path resolved by `tauri::path::app_config_dir`, e.g. `~/Library/Application Support/run.qlm.desktop/config.json` on macOS):

```json
{
  "QLM_SERVER_URL": "https://api.qlm.com",
  "QLM_TELEMETRY_ENABLED": "true",
  "OTEL_EXPORTER_OTLP_ENDPOINT": "https://otel.qlm.com",
...
| Where                                        | What                                                            | Lifetime               |
| -------------------------------------------- | --------------------------------------------------------------- | ---------------------- |
| OS keychain (`SERVICE_NAME = run.qlm.desktop`) | `refresh_token:<serverUrl>` — Supabase refresh token, one per profile | Across launches; deleted on sign-out / server change |
| OS keychain                                  | `MANAGED_KEYS` LLM provider keys                                | Across launches; deleted by user via Settings        |
| Tauri shell process memory                   | Per-launch one-time keyring-IPC bearer token (`QLM_KEYRING_TOKEN`) | One launch                                           |
...
| `~/.qlm/storage/` (local SQLite)         | Sidecar session cache, agent transcripts, datasource connection metadata | Until user clears                                |
...
- `useRuntime()` helper returning `'web' | 'desktop'`. Reads `window.__TAURI__` presence (exposed by Tauri webview) plus optional `window.__QLM_RUNTIME` injection from the sidecar.
...
- **Runtime detection.** Read `QLM_RUNTIME` at boot. When `desktop`, bind `127.0.0.1` only (never `0.0.0.0`); read `QLM_SERVER_URL` for the remote Supabase endpoint; read `QLM_REFRESH_TOKEN` (optional) and rehydrate session before serving the first request.
- **Keyring-IPC client.** Read `QLM_KEYRING_PORT` + `QLM_KEYRING_TOKEN` at boot. Provide a thin `keyringClient` module with `set` / `get` / `delete` calling the Tauri shell IPC.
- **Local SQLite app-state store.** `QLM_STORAGE_DIR` env var (default `~/.qlm/storage/`) for sidecar's session cache, agent transcripts, datasource metadata. Local-only, never synced.
...
- `src-tauri/tauri.conf.json` — **no `frontendDist`**; webview loads `http://127.0.0.1:<port>` via `window.__QLM_API_URL` injection. App identifier `run.qlm.desktop`. `bundle.externalBin` lists `binaries/api-server` per supported triple.
...
- `src-tauri/src/lib.rs` — lifted verbatim from qwery-core `lib.rs` with `SERVICE_NAME = "run.qlm.desktop"` and the qlm `MANAGED_KEYS` / `CONFIG_KEYS`. Retains: `target_triple()`, `configure_webview_zoom()`, `pick_port(4096)`, PID file, `kill_previous_api_server`, Windows env-subset (PATH/SystemRoot/TEMP/TMP/USERPROFILE/LOCALAPPDATA/APPDATA + `BUN_RUNTIME_TRANSPILER_CACHE_PATH=0`), env injection from keyring + `config.json`, server-ready TCP loop, child kill on window close + `before-quit`, `append_log_line` to `app_config_dir/desktop.log`. **Adds:** keyring IPC bind + one-time token generation, `QLM_REFRESH_TOKEN` injection at sidecar spawn.
...
- `src-tauri/src/ipc.rs` — keyring-IPC HTTP server. Endpoints `POST /keyring/set` / `GET /keyring/get` / `DELETE /keyring/{key}`. Bearer-token auth using `QLM_KEYRING_TOKEN`. Rate cap.
...
- [x] Same-origin webview ↔ sidecar at `127.0.0.1:<port>` (cookies scoped to that origin, never leave host). Shipped story 002; story 012 bridged apps/web via `window.__QLM_API_URL` preserving this.
- [x] Sidecar binds `127.0.0.1` only — assert in `apps/server` startup when `QLM_RUNTIME=desktop`. Shipped story 007 (`resolveDesktopRuntime` in `apps/server/src/runtime/desktop-runtime.ts`).
- [x] Keyring-IPC authenticated by per-launch one-time bearer token (`QLM_KEYRING_TOKEN`); token never logged. Shipped stories 004 + 007 (`ipc.rs` bearer check + `lib.rs` `redact_for_log` OnceLock).
- [x] Keyring-IPC rate-capped + clock-skew-checked (replay defence). Shipped story 004 (`ipc.rs` 100 req/s cap, 5s Date-skew guard).
- [x] Refresh token + LLM keys redacted from all logs (extend `append_log_line` filter list). Shipped story 004 (`redact_for_log` strips `Bearer <hex>` + the IPC token). LLM-key value redaction is defensive-by-construction: no call site logs them (values only logged as `=set|=empty|=missing|=error:<code>` markers per `lib.rs`).
- [x] HTTPS-only remote server URLs in production. `allowInsecureTls` requires explicit opt-in via `config.json`; production builds emit startup warning + Settings UI banner whenever it's on. Shipped story 009's Server pane banner. **Caveat**: story 012's Changelog (2026-04-23) surfaced a `tauri://localhost` mixed-content limitation — the prod `.app` cannot reach HTTP local Supabase at all (WebKit blocks HTTP from a secure origin); the HTTPS-only guarantee holds in practice regardless of `allowInsecureTls`.
- [x] OS keychain only — no fallback to plaintext file when keyring is unavailable; user is forced to re-auth that session. Shipped story 003 (`keyring_cmds` returns error on keychain failure; no disk fallback).
- [x] Tauri capabilities (`capabilities/default.json`) grant only what's listed in §7.8. Verified 2026-04-24: permissions are core window ops + `opener:default` + `dialog:default` + `os:default` + `shell:default` + `shell:allow-execute` scoped to `binaries/api-server` (sidecar). No broader shell access.
- [x] MDM `config.json` integrity is **NOT** verified in phase 1 — documented as "trust the OS file-permission boundary"; revisit in a later phase if customers want signed config. See `docs/desktop-mdm.md` §Security notes.
- [x] System-browser handoff for OAuth providers is deferred — no in-webview third-party OAuth screens. Verified: no OAuth screens shipped across 001–012.
- [x] Residual threats called out: compromised host with keychain access, malicious local process scanning loopback ports (mitigated by IPC token check + binding `127.0.0.1` only). Verified.
...
- **Bun/TS** (`apps/server` desktop-runtime tests): startup logic when `QLM_RUNTIME=desktop` (binds 127.0.0.1, reads `QLM_REFRESH_TOKEN` env, calls Supabase refresh, writes new token back via keyring IPC). Mock the IPC server with `msw` or a small stub.
...
- For phase 1, add `apps/web` Playwright tests that stub `window.__TAURI__` + `window.__QLM_RUNTIME='desktop'` and exercise: first-run picker visibility, Server pane visibility, LLM keys pane visibility, runtime-gated routing.
...
6. Verify keychain via `security find-generic-password -s run.qlm.desktop` (macOS) — see `refresh_token:<serverUrl>` entries per profile.
...
- `apps/server` reads `QLM_RUNTIME=desktop` at boot — binds `127.0.0.1` only, reads `QLM_SERVER_URL`, reads `QLM_STORAGE_DIR`.
...
- Rust keyring-IPC HTTP server: `/keyring/set` / `/keyring/get` / `/keyring/{key}` with `QLM_KEYRING_TOKEN` bearer auth + rate cap.
- `apps/server` keyring-IPC client: read `QLM_KEYRING_PORT` + `QLM_KEYRING_TOKEN`, expose `keyringClient.{set,get,delete}`.
- `apps/web` Settings → LLM keys pane (gated by `runtime === 'desktop'`).

Stage D — Auth wiring + first-run + server switcher
- `apps/server` boot path: read `QLM_REFRESH_TOKEN`, refresh against Supabase, write new token back via `keyringClient.set`.
...
- Cloud LLM proxy + `qlmCloud()` `LanguageModelV2` provider — replaces BYO keys for hosted users.
...
- `qlm://` custom protocol handler.
- Certificate pinning for on-prem.
- Device attestation / binding.
- Real Tauri WebDriver e2e suite.
- MDM `config.json` integrity verification (signed config support). The **format + delivery mechanism** ship in phase 1 — see `docs/desktop-mdm.md` for the config-file shape, per-OS paths, and MDM how-to.
...
- 2026-04-19 — story 001 — `tauri.conf.json` ships with `frontendDist: "../dist"` (the desktop SPA build output) instead of "no frontendDist". The bundled `.app` needs a renderable shell even pre-sidecar; qwery-core uses both static frontendDist AND sidecar URL injection at runtime, and we adopt the same pattern.
- 2026-04-19 — story 001 — added task 006 `wrap-client-only-spa`. `apps/web` is a TanStack Start SSR app and has no static `index.html`, so it cannot be served as Tauri's `frontendDist`. Story 001 now ships a thin client-only TanStack Router SPA inside `apps/desktop/src/` (mirroring qwery-core's pattern). Vite alias `@/` → `apps/web/src` is wired for future stories to pull shared route components.
- 2026-04-19 — story 011 — §0 said "the renderer (apps/web) is reused unchanged"; in practice the curated re-export pattern is what ships. Pointing apps/desktop's tanstackRouter plugin at apps/web/src/routes/ pulls the entire workspace dep graph in (`agent-factory-sdk` uses `node:fs/promises`, `packages/supabase` uses `node:process`, billing uses Stripe SDK with Node imports) which Vite cannot bundle for the browser. apps/desktop now owns its own small route tree under `apps/desktop/src/routes/`; routes that need to look like apps/web import the **components** (browser-safe) from `apps/web/src/components/` via the `@/` alias and redefine the route container locally. Each desktop-shipped route is opt-in.
- 2026-04-23 — story 012 — added a 12th phase-1 story not in the original `/spec-to-stories` slice. Story 011's curated re-export shipped only the first-run picker; the rest of the QLM console (sign-in, orgs, projects, settings, server pane) was unreachable from the packaged `.app`. Story 012 reverses 011's deviation by enabling TanStack Start's additive SPA shell (`spa: { enabled: true, prerender: { outputPath: '/index' } }`), which emits `apps/web/dist/client/index.html` alongside the existing SSR build. Tauri now points at apps/web (`devUrl: localhost:3000`, `frontendDist: ../../web/dist/client`); apps/desktop's placeholder SPA stays on disk but no longer renders. apps/web's `getApiBaseUrl()` reads `window.__QLM_API_URL` so all `/api/*` calls reach the Bun sidecar regardless of webview origin. **Known limitation surfaced**: WebKit treats `tauri://localhost` as a secure context, so the prod `.app` cannot reach **HTTP** local Supabase due to mixed-content rules — flagged for a future "sidecar-owned auth" story (the spec's §3.3 + §5.2 sidecar `/auth/sign-in` route is built in story 007 but apps/web's SPA still calls Supabase JS directly). Cloud HTTPS Supabase works in prod; dev mode (`tauri:dev`) works against any HTTP target.
- 2026-04-24 — story 006 — moved out of this spec. BYO LLM keys management has enough independent surface area (provider list, web-vs-desktop split, security controls, cloud-proxy handoff) to warrant its own RFC. Story 006 is `[skipped]`; the feature continues in `docs/rfcs/0026-llm-keys-management.md`. The Rust-side scaffolding already built (`keyring_cmds`, `MANAGED_KEYS`, keyring-IPC listener, shell-runtime `saveProviderKey` helpers) is reused by 0026 — not replaced. Story 010's `blocked_by` list dropped `006`.
