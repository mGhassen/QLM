...
- [ ] `apps/desktop/src-tauri/tauri.conf.json` has `productName: "QLM Desktop"`, `identifier: "run.qlm.desktop"`, `version: "0.1.0"`, **no `frontendDist`** (the sidecar will inject `window.__QLM_API_URL`), one window with sane defaults (1440×900, min 1024×720, dark background `#111827`).
