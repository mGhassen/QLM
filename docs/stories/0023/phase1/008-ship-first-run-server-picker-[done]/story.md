...
**In scope**
- New file-based route `apps/web/src/routes/desktop/first-run.tsx`. Visible only when `runtime === 'desktop'` AND `getAppConfig()` lacks `QLM_SERVER_URL`.
- Layout: centred card on full-page background. Two radio options (Cloud (qlm.com) default-selected, Custom server URL). Selecting Custom reveals a text input with TLS validation on blur.
- MDM-default banner when `getAppConfig()` returns a `QLM_SERVER_URL` written by MDM (file mtime older than first-launch marker, per spec §9 — implementation may use a flag stored at first launch).
- "Continue" CTA disabled until selection is valid; on click → `setAppConfig({ QLM_SERVER_URL: <url> })` then `restartSidecar()` (calls the wrapper from story 005; the underlying Tauri command is a stub for now and will be fully implemented in story 009).
...
- [x] `pnpm typecheck` 51/51 green; ESLint `@qlm/ui/trans` rule respected; all visible strings via `t(...)` against the new `desktop` namespace.
- [x] On `runtime === 'web'`, the route is unreachable — `beforeLoad` in the route file redirects to `/` when `isDesktopApp() === false`.
- [x] Selecting Cloud → Continue persists `QLM_SERVER_URL=https://api.qlm.com` to `app_config_dir/config.json` via `setAppConfig` from `@qlm/shell-runtime`. Verified by the picker's `submits the cloud URL` unit test.
