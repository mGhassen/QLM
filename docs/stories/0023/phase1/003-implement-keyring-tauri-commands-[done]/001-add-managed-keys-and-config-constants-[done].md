...
- [ ] New `apps/desktop/src-tauri/src/keys.rs` exposes:
  ...
  - `pub const CONFIG_KEYS: &[&str] = &[...]` — `GUEPARD_SERVER_URL`, `GUEPARD_TELEMETRY_ENABLED`, `OTEL_EXPORTER_OTLP_ENDPOINT`, exactly as listed in spec §7.8.
...
- Reference: `qwery-core/apps/desktop/src-tauri/src/lib.rs` lines for the `MANAGED_KEYS` + `CONFIG_KEYS` arrays. Don't lift qwery-specific keys (`QWERY_*`) — use the guepard list per the spec.
