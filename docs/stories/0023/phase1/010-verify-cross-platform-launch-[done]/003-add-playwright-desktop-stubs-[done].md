...
Per spec §10.4: real Tauri WebDriver e2e is deferred; for phase 1, add Playwright tests that stub `window.__TAURI__` + `window.__GUEPARD_API_URL` so the runtime-gated routes render in a normal browser.
...
- [ ] `apps/e2e/tests/desktop/desktop-runtime-gates.spec.ts` (new) with one spec:
  - `first-run picker renders under desktop runtime`: `page.addInitScript` injects `window.__TAURI__ = {}` + `window.__GUEPARD_API_URL = 'http://127.0.0.1:3999'` before navigation. Navigate to `/desktop/first-run`. Assert the picker's three options (Cloud / Custom / Local) render + a `Continue` button is visible.
...
- `isDesktopApp()` from `@guepard/shared/desktop` just checks `typeof window.__TAURI__ !== 'undefined'`; no methods need to be populated.
