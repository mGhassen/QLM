...
Per story §in-scope: a new `docs/desktop-mdm.md` for IT admins rolling out QLM Desktop via MDM (Jamf / Intune / silent provisioning). Pin down `config.json` shape + per-OS file paths.

## Done when

- [ ] `docs/desktop-mdm.md` (new) covers:
  - Purpose (one paragraph: pre-seed `config.json` so users don't see the first-run picker).
  - `config.json` schema with a sample (`QLM_SERVER_URL`, `QLM_SERVER_MDM_DEFAULT`, `QLM_TELEMETRY_ENABLED`, `OTEL_EXPORTER_OTLP_ENDPOINT`, `allowInsecureTls`).
  ...
  - How `QLM_SERVER_MDM_DEFAULT` surfaces in the UI (the MDM badge in the Server pane; the banner in first-run).
