...
- [ ] `ShellAppProvider` uses `useEffect` to call `setLastProject(orgSlug, projectId)` once per mount and whenever either key changes. Errors caught → `@guepard/shared/logger`.warn; never rethrown.
...
- `@guepard/shared/logger` is async (pino under the hood); use `void (async () => { … })()` if needed to keep the effect synchronous.
