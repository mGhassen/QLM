...
model: sonnet
files:
  - packages/ui/src/guepard/shell/project-shell-tab-bar.test.tsx
validation:
  kind: domain-test
  specs:
    - packages/ui/src/guepard/shell/project-shell-tab-bar.test.tsx
...
- [ ] `pnpm --filter @guepard/ui test packages/ui/src/guepard/shell/project-shell-tab-bar.test.tsx` is green.

## Notes

- If `/finish` rejects `kind: domain-test` because the runner targets `@guepard/domain` only, fall back to `kind: typecheck-only` and run the test in the story demo block.
