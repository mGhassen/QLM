...
Land the UI-visible MFA card below `PasswordCard`. Reads existing factors via `useFetchAuthFactors()` from `@qlm/supabase` (already in repo). Renders the empty-state callout when no factors are enrolled, or a list of factor names when there are. *Setup a new Factor* button is **disabled** in this task — task 003 wires the enrollment dialog.
...
- `useFetchAuthFactors()` lives at `packages/supabase/src/hooks/use-fetch-mfa-factors.ts`; add `@qlm/supabase` as a dep on the user-profile package if not already there.
