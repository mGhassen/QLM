...
- `packages/features/user-tokens/src/components/token-row.tsx`:
  ...
  - Wraps a `<tr>` with 7 `<td>` children matching spec §3.2.1:
    ...
    7. Actions → `@qlm/ui/button` with `variant="ghost"` + `size="icon"` + `Trash2` lucide icon. Disabled when `deriveUserTokenStatus(token) !== 'active'`. `aria-label={t('tokens:table.revokeAriaLabel')}`. `onClick={() => onRevokeClick(token)}`.
- `packages/features/user-tokens/src/components/token-row.stories.tsx`:
