...
Implement `JwtSigner extends IJwtSigner` that wraps `jsonwebtoken.sign` with HS256. Isolated from the adapter so `@supabase/*` and `jsonwebtoken` stay outside `@guepard/domain`.
...
- `packages/repositories/supabase/package.json` — add `jsonwebtoken` (`^9.0.2`) + `@types/jsonwebtoken` (`^9.0.7`) deps. Shared version with `@guepard/auth-shared`.
...
- The `@guepard/auth-shared` package also depends on `jsonwebtoken@^9.0.2` (Story 007); version pinned so sig formats stay identical across the two packages.
