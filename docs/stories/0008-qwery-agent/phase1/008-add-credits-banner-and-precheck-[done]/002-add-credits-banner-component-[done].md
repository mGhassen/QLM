...
- `packages/features/qwery-agent/src/credits-banner.tsx` — **new**. Exports `CreditsBanner({ orgSlug }: { orgSlug: string })`. Renders using `react-i18next`'s `useTranslation()` with namespace `chat`. Structure: title + description + `<a>` CTA linking to `/org/${orgSlug}/billing`. Visual: reuse `@guepard/ui/alert` for consistency.
...
- [ ] `CreditsBanner` exported from `@guepard/qwery-agent`.
