...
- [x] Uses `normalizeUIRole` and `messageRoleToUIRole` from `@qlm/shared/message-role-utils` (no inline role mapping).
- [x] `pnpm typecheck` passes. Required adding a local `src/types/turndown.d.ts` shim because qwery-agent's tsc walks into agent-factory-sdk's source (which transitively imports `turndown`) and doesn't pick up the sibling shim there. Also added `@qlm/agent-factory-sdk`, `@qlm/domain`, `@qlm/shell-runtime` to deps and `@tanstack/react-query` to devDeps.
...
- Keep the helper feature-local (not in `@qlm/shared`) — no other consumer in phase 1.
- `UIMessage` type is exported from `@qlm/agent-factory-sdk` (re-exported from `ai`).
