...
# Expose getAuthHeaders from @guepard/supabase
...
- [ ] `pnpm --filter @guepard/supabase typecheck` passes.
...
- Do not duplicate `getAuthHeaders` from `apps/web/src/lib/repositories/api-client.ts`. A later cleanup can switch that file to import from `@guepard/supabase/auth-headers`, but that refactor is out of this task's scope.
