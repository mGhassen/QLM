...
# Rename @qlm/nodes → @qlm/infrastructure and absorb the dying pkg

## Goal

Single canonical Infrastructure package. Rename `@qlm/nodes` to `@qlm/infrastructure`, absorb the activity + settings + replicas surfaces from the dying `@qlm/infrastructure`, delete cluster/provider/overview tabs (covered now by Topology + Pool entity), and update every consumer.
...
- Rename `package.json:name` → `@qlm/infrastructure`. Add `exports['./types']: './src/application/types.ts'` to preserve the MSW import path.
...
- Update `packages/apps/infrastructure/package.json` dep `@qlm/nodes` → `@qlm/infrastructure`.
- Update `apps/web/package.json` workspace deps. The dying `@qlm/infrastructure` dep already exists; after rename it points at the merged pkg. Confirm version pin.
...
- [ ] `packages/features/ops/nodes/` no longer exists. `packages/features/ops/infrastructure/` is the merged pkg with `package.json:name = '@qlm/infrastructure'`.
- [ ] `grep -rn "@qlm/nodes" packages apps tooling` returns zero hits.
- [ ] `apps/web/src/lib/msw/handlers/replicas.ts` and `apps/web/src/lib/msw/fixtures/infrastructure.ts` keep their existing `import type … from '@qlm/infrastructure/types'` lines unchanged. They resolve correctly.
