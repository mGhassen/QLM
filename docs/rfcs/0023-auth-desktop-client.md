...
- [ ] Sidecar binds `127.0.0.1` only — assert in `apps/server` startup when `QLM_RUNTIME=desktop`.
- [ ] Keyring-IPC authenticated by per-launch one-time bearer token (`QLM_KEYRING_TOKEN`); token never logged.
