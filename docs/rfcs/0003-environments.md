...
- **Loading**: skeletons using `@guepard/ui/skeleton` (or equivalent). Full-card skeletons for Layer 1; graph-level skeleton for Layer 2.
...
- `StatusChip` — the shared status-chip primitive (or a thin wrapper around `@guepard/ui/badge`).
...
Every component is a function component, uses `Readonly<Props>`, localizes every string via `t(...)` or `@guepard/ui/trans`.
...
- **No network calls**: phase-1 components do not import from `@guepard/shell-runtime`. The only imports are `@guepard/ui/*`, `@guepard/i18n`, `react`, `@tanstack/react-router`, and the local fixture module.
...
- Every user-facing string goes through `t(...)` or `@guepard/ui/trans`. No hardcoded English. ESLint enforces `@guepard/ui/trans` over `react-i18next/Trans`.
...
- **Layer 1 also lists top-level Guepard databases with no source.** Rejected in Round 1. Mixes two lifecycles in one view.
...
- **Drop the `is_source` column and make Layer 1 list every datasource.** Rejected. Conflates "a datasource I use as a qwery query target" with "a datasource I want to ingest into Guepard". Users would see every connection string they have ever entered under the environments app, which is wrong both conceptually and visually.
