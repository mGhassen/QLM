...
Introduce **Integrations** — a project-scoped primitive that holds a user's credentials to a third-party cloud (AWS, GCP first; later Supabase, Neon, Azure, …) and exposes a small, growing driver surface through which QLM performs cloud-side operations on the user's behalf.

Integrations are the credential and permission boundary that unlocks QLM's two defining capabilities against the user's own infrastructure:

1. **Dataplane private nodes** — provision QLM compute inside the user's cloud account so that git-like databases and query execution live next to (and inside the network of) the user's data, with no egress out of their VPC.
2. **Cloud database ingestion** — discover the user's existing managed databases (RDS, Cloud SQL, Aurora snapshots, …) and pull them into QLM as git-like databases: the first ingest becomes the root commit, subsequent ingests become branches or new commits.
...
QLM is a data platform built around **git-like databases** — databases the user can branch, commit, diff, time-travel, and hand to AI agents — plus query tooling and agent connectivity on top. The versioning layer (GFS) and the git-like database primitives live outside this console today; what the console is missing is the *bridge* between a user's existing cloud infrastructure and that platform.

Concretely, two things the product needs to do that it cannot do today:

1. **Run QLM inside the user's cloud.** Many prospective users cannot move data across a network boundary for compliance, cost, or latency reasons. QLM's answer is the **dataplane private node**: a QLM runtime (compute + storage locally close to the user's databases) provisioned inside the user's own AWS / GCP account and registered with the control plane. The user gets git-like versioning and agent access without ever shipping bytes outside their network. To stand up a private node, QLM needs credentials on the user's cloud account, region awareness, and the ability to call provisioning APIs. That all starts with the integration primitive.

2. **Ingest an existing cloud database into QLM.** The value proposition of QLM ("your Postgres, but with branches") is realised only when a user can point QLM at their RDS instance or RDS snapshot and say *"make this a git-like database"*. The ingestion flow — discover RDS instances, pick one, snapshot it, pull the snapshot into the dataplane, register it as a git-like database rooted at commit zero — all depends on QLM having credentials and cloud-API reach into the user's account. That, again, starts with the integration primitive.

A secondary, but important, consequence: **AI agents query databases, not clouds**. Agents in `packages/agent-factory-sdk` connect to registered databases in QLM by id. They do not and should not know about cloud credentials. Integrations are the upstream primitive that lets "a cloud database" *become* "a QLM git-like database" that the agent can then use. The agent layer is a consumer of what integrations unlock; it is not a consumer of integrations themselves.
...
Relevant existing pieces that shape this design. It is important to be clear which are legacy qwery primitives, which are still core to QLM, and which are orthogonal.
...
- `test_identity` is stored on successful tests so the list view can show "connected as arn:aws:iam::123:user/qlm" or the GCP service-account email without re-hitting the provider.
...
- **Residual threat — server compromise**: a compromised QLM server with vault decryption access can drain every credential in every project. This is the biggest argument for OIDC federation (phase 4), not a phase-1 mitigation.
...
- No hardcoded strings in JSX, toasts, aria labels, placeholders, or error messages. All user-facing text goes through `t(...)` or `@qlm/ui/trans`, per `.claude/rules/i18n.md`.
...
- **Skip the integration primitive and hardcode a QLM-owned AWS account for provisioning.** Rejected. It does not solve the core problem: users need nodes running in *their* cloud accounts for compliance and data-locality reasons. A QLM-owned cloud account is a demo fixture, not the product.
...
- [ ] Does §4 distinguish qwery-era primitives (`datasources`, `notebooks`) from the QLM primitives (git-like databases, dataplane nodes, ingestion) clearly enough that no reader walks away thinking integrations are a datasource subtype?
...
RFC §UX sketched a bespoke header + grid pair. After design review (consistency with datasources, notebooks, nodes) the list page was rewritten on top of the shared `@qlm/ui/entity-list`'s `EntityListPage` wrapper, which owns search / sort / grid-vs-table toggle. Net effect: one less bespoke header component and consistent UX with every other project-scoped list in the console.
