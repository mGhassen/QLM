# Guepard Desktop — MDM configuration

This document is for IT administrators rolling out Guepard Desktop via MDM (Jamf, Intune, Workspace ONE, or equivalent). It documents the `config.json` format and file location for silent provisioning — so end-users never hit the first-run picker.
...
| Key                          | Type      | Purpose                                                                                                   |
| ---------------------------- | --------- | --------------------------------------------------------------------------------------------------------- |
| `GUEPARD_SERVER_URL`         | URL       | Which Guepard server the sidecar talks to (cloud or on-prem). When set, the first-run picker is skipped. |
| `GUEPARD_SERVER_MDM_DEFAULT` | URL       | Same URL as above, but also displays the "Set by your IT team" badge in Settings → Server.              |
| `GUEPARD_TELEMETRY_ENABLED`  | `"true"` / `"false"` | Toggles OpenTelemetry emission in the sidecar.                                                |
...
```json
{
  "GUEPARD_SERVER_URL": "https://guepard.acme-corp.example",
  "GUEPARD_SERVER_MDM_DEFAULT": "https://guepard.acme-corp.example",
  "GUEPARD_TELEMETRY_ENABLED": "true",
  "OTEL_EXPORTER_OTLP_ENDPOINT": "https://otel.acme-corp.example"
}
```

Recommended: set both `GUEPARD_SERVER_URL` and `GUEPARD_SERVER_MDM_DEFAULT` to the same value so users see the MDM badge + can't accidentally connect to a different server without understanding what changed.
...
The app reads the file once per launch. When `GUEPARD_SERVER_URL` is set, the first-run picker is skipped and the sidecar starts directly against that URL.

## Delivery — Jamf Composer

1. Create a new package source with a "fill existing users" template.
2. Write `config.json` to `/Library/Application Support/run.guepard.desktop/config.json` (system-wide) **or** `/Users/Shared/...` and drop a login hook that copies it into each user's `~/Library/Application Support/run.guepard.desktop/`.
3. Scope the policy to the target smart group; trigger on first login.

Apple's recommended pattern is to deploy to `~/Library/Application Support/...` per-user rather than `/Library/...`, because Guepard Desktop reads the per-user path.

## Delivery — Intune (Windows)

1. In Endpoint Manager, create a Win32 app with an install script that drops `config.json` into `%APPDATA%\run.guepard.desktop\config.json` under the target user.
...
```bash
mkdir -p "$HOME/.config/run.guepard.desktop"
cat > "$HOME/.config/run.guepard.desktop/config.json" <<'EOF'
{
  "GUEPARD_SERVER_URL": "https://guepard.acme-corp.example",
  "GUEPARD_SERVER_MDM_DEFAULT": "https://guepard.acme-corp.example"
}
EOF
chmod 600 "$HOME/.config/run.guepard.desktop/config.json"
```
...
Setting `"allowInsecureTls": "true"` lets the app connect to `http://` Guepard servers (typically `http://127.0.0.1:54321` against a dev Supabase). When this is on:
...
- File integrity is **not** cryptographically verified in phase 1. Guepard Desktop trusts the OS file-permission boundary: if an attacker has write access to the user's `~/Library/...` (macOS), `%APPDATA%\...` (Windows), or `~/.config/...` (Linux), they can already subvert many things. Signed-config is a phase-2 follow-up listed in `docs/specs/0023-auth-desktop-client-phase1.md` §13.
...
- **User still sees the first-run picker** — verify the file exists at the correct per-user path (not the system-wide `/Library/...`) and contains a valid `GUEPARD_SERVER_URL`. Check `desktop.log` in the same directory for JSON parse errors.
- **"Insecure TLS allowed" banner unexpectedly visible** — `allowInsecureTls` is `"true"`. Remove it or set to `"false"`.
- **Server URL doesn't show the MDM badge** — `GUEPARD_SERVER_MDM_DEFAULT` must exactly match `GUEPARD_SERVER_URL` (trailing slash included).
